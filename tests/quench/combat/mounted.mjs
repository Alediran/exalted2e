...
export function registerMounted(context) {
  ...
  describe("Mounted combat — Ride cap on DVs", function () {
    ...
    // Dex 4, Dodge 4 → dodgeDV = floor((4+4)/2) = 4 uncapped.
    // Ride 2 cap → floor((4 + min(4,2)) / 2) = floor(6/2) = 3.
    it("[M-1] currentDodgeDV is capped by Ride when mountedOn flag is set",
      async function () {
        this.timeout(15000);
    ...
    it("[M-2] currentDodgeDV returns to normal after dismounting",
      async function () {
        this.timeout(15000);
    ...
    it("[M-3] Ride cap is a no-op when Ride >= Dodge",
      async function () {
        this.timeout(15000);
        // Dex 4, Dodge 2, Ride 4 — Ride is higher than Dodge, cap does not fire.
    ...
    // M-4: Dodge DV is not capped outside of combat (no combatant to check).
    it("[M-4] currentDodgeDV is not capped when actor has no active combat",
      async function () {
        this.timeout(15000);
        // No combat started — game.combat should be null or this actor is not a combatant.
        const actor  = await createMountedActor("Mounted-DV-4", { dex: 4, dodge: 4, ride: 2 });
        const dvFull = actor.currentDodgeDV;
        // With Dex 4, Dodge 4 → dodgeDV = 4.  Without a combat / mounted flag the cap must not fire.
        assert.equal(dvFull, 4,
          "Dodge DV is full when not in combat (no mountedOn combatant)");
      }
    );

    // M-5: currentParryDV is capped by a low Ride value.
    // Use Ride=1 to guarantee a visible gap regardless of the unarmed weapon defense modes:
    //   rideCap = floor((Dex + min(Melee, Ride=1) + weaponDef) / 2)
    //           ≤ floor((4 + 1 + 0) / 2) = 2
    // which is always below s.parryDV for a character with Dex 4, Melee 4.
    it("[M-5] currentParryDV is capped when Ride is very low (Ride=1)",
      async function () {
        this.timeout(15000);
        const actor = await createMountedActor("Mounted-ParryDV", { dex: 4, melee: 4, ride: 1 });
        const sc    = getTestScene();
        await placeToken(actor, sc, { x: 0, y: 0 });
        const combat = await startTempCombat([actor]);
        ...
        assert.isBelow(dvMounted, dvNormal,
          `Parry DV (${dvMounted}) should be lower than uncapped (${dvNormal}) with Ride 1`);
      }
    );
  });
}
