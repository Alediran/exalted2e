...
- [~] **Coverage-raising initiative** — lift true combined (Vitest∪Quench) coverage from ~67% lines, worst-first per directory. Strategy: extract pure logic → Vitest + thin Quench happy-path smoke. **All 5 directory areas done (SP1–SP5) + SP2b; only the regression-floor CI gate remains.** Specs/plans under `docs/superpowers/{specs,plans}/2026-06-06-coverage-sp*`.
  ...
  - [ ] Regression-floor gate on the CI `coverage` job — the only remaining initiative item (SP1–SP5 + SP2b all done)
