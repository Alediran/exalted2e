  ...
  static async prompt({ active }) {
    ...
    return foundry.applications.api.DialogV2.prompt({
      ...
      ok: {
        ...
        callback: (_ev, button) => {
          const val = button.form?.elements?.tierSelection?.value ?? "standard";
          if (!val || val === "standard") return { standard: true };
          const idx = parseInt(val.replace("tier_", ""), 10);
          if (isNaN(idx) || idx < 0 || idx >= active.length) return { standard: true };
