...
export class HazardDamageBehaviorType extends foundry.data.regionBehaviors.RegionBehaviorType {
  ...
  static async #applyOrPrompt(tokenDoc) {
    ...
    // Check for hazard immunity from an active Charm effect.
    // Charm effects set flags.exalted2e.hazardImmunity = "natural" | "supernatural" | "all".
    const immunity = actor.effects.find(e => !e.disabled && e.flags?.exalted2e?.hazardImmunity);
    if (immunity) {
      const scope = immunity.flags.exalted2e.hazardImmunity;
      const isImmune = scope === "supernatural"
        || (scope === "natural" && !this.system.isSupernatural);
      if (isImmune) {
    ...
    if (actor.hasPlayerOwner && this.system.resistDifficulty > 0) {
      await HazardDamageBehaviorType.#postResistanceCard.call(this, actor, tokenDoc);
    } else {
  ...
  static async #postResistanceCard(actor, tokenDoc) {
    const hazardName = this.parent?.parent?.name ?? "Hazard";
