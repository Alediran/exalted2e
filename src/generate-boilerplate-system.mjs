...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'exalted2e', 'ExaltedSecond', and 'EXALTED2E'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.exalted2e/g),
        replacement: `game.${this.propName}`,
      },
      {
        pattern: new RegExp(/flags\.exalted2e/g),
   ...
   * Rename files.
   *
   * Rename files that had exalted2e in their name, such as
   * css/exalted2e.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*exalted2e*.*`).then((files) => {
      files.forEach((oldPath) => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(
          oldPath,
          `${directory}/${file.replaceAll("exalted2e", this.packageName)}`,
          (err) => {
            if (err) throw err;
          }
        );
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-exalted2e-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob ExaltedSecond's files so that we can process them.
    glob("*", { ignore: ["node_modules/**"] }).then((files) => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace exalted2e name mentions in files.
      generator.replaceFileContents();
      // Rename files that had exalted2e in their name.
